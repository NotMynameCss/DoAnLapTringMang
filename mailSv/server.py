...
def process_message(message, main_controller, mail_controller, client_addr):
    ...
    try:
        if message.startswith("REGISTER"):
            ...
            logger.info(f"[{action}] user={user} ip={ip}")
            # Nếu email_type là "all", lấy mọi email liên quan user (sender, recipients, cc, bcc)
            if email_type == "all":
                emails = mail_controller.fetch_emails_by_user(username, email_type="all")
            else:
            ...
            # Không cho phép client lấy toàn bộ email hệ thống
            action = "FETCH_ALL_EMAILS"
            logger.warning(f"[{action}] user={user} ip={ip} - BỊ TỪ CHỐI: Không cho phép lấy toàn bộ email hệ thống")
            response = json.dumps({"success": False, "message": "Không được phép lấy toàn bộ email hệ thống"})
